...
WindowRegressionDataLayer<Dtype>::~WindowRegressionDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void WindowRegressionDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  do {
    ...
    for (int i = 0; i < num_windows; ++i) {
      int x1, y1, x2, y2;
      float label; // added by rg
      float overlap;
      infile >> label >> overlap >> x1 >> y1 >> x2 >> y2;

      vector<float> window(WindowRegressionDataLayer::NUM);
      window[WindowRegressionDataLayer::IMAGE_INDEX] = image_index;
      window[WindowRegressionDataLayer::LABEL] = label;
      window[WindowRegressionDataLayer::OVERLAP] = overlap;
      window[WindowRegressionDataLayer::X1] = x1;
      window[WindowRegressionDataLayer::Y1] = y1;
      window[WindowRegressionDataLayer::X2] = x2;
      window[WindowRegressionDataLayer::Y2] = y2;

      // add window to foreground list or background list
      if (overlap >= fg_threshold) {
        float label = window[WindowRegressionDataLayer::LABEL];
        CHECK_GT(label, 0);
        fg_windows_.push_back(window);
      } else if (overlap < bg_threshold) {
        // background window, force label and overlap to 0
        window[WindowRegressionDataLayer::LABEL] = 0;
        window[WindowRegressionDataLayer::OVERLAP] = 0;
...
template <typename Dtype>
unsigned int WindowRegressionDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
void WindowRegressionDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for (int is_fg = 0; is_fg < 2; ++is_fg) {
    for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
      ...
      pair<std::string, vector<int> > image =
          image_database_[window[WindowRegressionDataLayer<Dtype>::IMAGE_INDEX]];

      cv::Mat cv_img;
      if (this->cache_images_) {
        pair<std::string, Datum> image_cached =
          image_database_cache_[window[WindowRegressionDataLayer<Dtype>::IMAGE_INDEX]];
      ...
      int x1 = window[WindowRegressionDataLayer<Dtype>::X1];
      int y1 = window[WindowRegressionDataLayer<Dtype>::Y1];
      int x2 = window[WindowRegressionDataLayer<Dtype>::X2];
      int y2 = window[WindowRegressionDataLayer<Dtype>::Y2];
      ...
      // get window label
      top_label[item_id] = window[WindowRegressionDataLayer<Dtype>::LABEL];
      ...
      inf << image.first << std::endl
          << window[WindowRegressionDataLayer<Dtype>::X1]+1 << std::endl
          << window[WindowRegressionDataLayer<Dtype>::Y1]+1 << std::endl
          << window[WindowRegressionDataLayer<Dtype>::X2]+1 << std::endl
          << window[WindowRegressionDataLayer<Dtype>::Y2]+1 << std::endl
          << do_mirror << std::endl
...
INSTANTIATE_CLASS(WindowRegressionDataLayer);
REGISTER_LAYER_CLASS(WindowRegressionData);
